...
def duration_to_seconds(value: str) -> int:
    """
    Suporta formatos simples do k6:
    30s, 1m, 1m30s, 2h, 1h10m30s
    """
    ...
    if not match:
        raise ValueError(f"Duração inválida: {value}")
    ...
    if total <= 0:
        raise ValueError(f"Duração precisa ser maior que zero: {value}")

    return total


def seconds_to_k6_duration(seconds: int) -> str:
    if seconds < 60:
        return f"{seconds}s"
...
""".strip()
        ...
        constant_duration_seconds = duration_to_seconds(duration)

        rampup_duration_seconds = 45
        spike_duration_seconds = 30
        graceful_stop_seconds = 30

        rampup_start_seconds = constant_duration_seconds + graceful_stop_seconds
        spike_start_seconds = rampup_start_seconds + rampup_duration_seconds + graceful_stop_seconds

        rampup_start = seconds_to_k6_duration(rampup_start_seconds)
        spike_start = seconds_to_k6_duration(spike_start_seconds)
...
export default function () {{
  ...
  const decision = message.overallDecision;
...
def build_detail_html(run_dir: Path, metadata: RunMetadata, rows: list[dict[str, Any]]) -> None:
    ...
    table_rows = "\n".join(
        f"<tr>"
        f"<td>{escape(r['second'])}</td>"
        f"<td>{r['total']}</td>"
        f"<td>{r['allowed_ok']}</td>"
        f"<td>{r['rate_limited']}</td>"
        f"<td>{r['other']}</td>"
        f"</tr>"
        for r in rows
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Executa testes k6 via gRPC e gera relatórios HTML."
    )
    ...
    run_parser.add_argument(
        ...
        help="Host gRPC do klimiter-service.",
    )
    run_parser.add_argument(
        "--key",
        default="user_id",
        help="Nome da chave de rate limit. Exemplo: user_id.",
    )
    run_parser.add_argument(
        "--value",
        default="test-user",
        help="Valor da chave de rate limit. Exemplo: test-user.",
    )
    run_parser.add_argument(
        "--cost",
        type=int,
        default=1,
        help="Custo da requisição.",
    )
    run_parser.add_argument(
        "--scenario",
        choices=["constant", "rampup", "spike", "mixed"],
        default="constant",
        help="Cenário de carga do k6.",
    )
    run_parser.add_argument(
        "--rate",
        type=int,
        default=300,
        help="Requisições por segundo alvo.",
    )
    run_parser.add_argument(
        "--duration",
        default="30s",
        help="Duração do teste no formato do k6. Exemplo: 30s, 1m, 1m30s.",
    )
    run_parser.add_argument(
        "--pre-vus",
        type=int,
        default=100,
        help="preAllocatedVUs do k6.",
    )
    run_parser.add_argument(
        "--max-vus",
        type=int,
